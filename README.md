...
RAG Forge is a **full-stack, production-ready Retrieval-Augmented Generation (RAG) platform**. It lets you turn your own documents into a private, enterprise-grade AI knowledge base — with no cloud vendor lock-in and no compute costs.

Here's the simple version: you upload PDFs, Word docs, text files, or HTML pages. RAG Forge reads them, breaks them into meaningful chunks, turns those chunks into searchable vectors, and stores everything in a local database. From there, you can ask questions in plain English through a chat interface and get back answers that are **grounded only in your documents** — complete with **source citations** and **confidence scores**. No made-up facts, no data leaving your machine.
...
### 🏗 Full RAG Pipeline
- **Multi-format ingestion** — reads PDF (PyMuPDF), DOCX (python-docx), TXT, and HTML (BeautifulSoup)
- **Smart, sentence-aware chunking** with adjustable chunk size and overlap
- **Local vector embeddings** via `sentence-transformers` (384 dimensions, runs on CPU — no GPU needed)
- **ChromaDB** as a persistent vector store with cosine similarity search
- **BM25 keyword search**, with optional hybrid search that blends keyword + vector results
- **Cross-encoder re-ranking** (`ms-marco-MiniLM-L-6-v2`) to push the most relevant chunks to the top
- **Grounded LLM answers** — the model is restricted to answering only from the retrieved context

### 🤖 Flexible LLM Options
- **Ollama** (default) — fully local, completely free, supports Llama 3.2, Mistral, Phi-3, and any model you've pulled
...
- Each user only sees their own collections and documents
- OTP-based password reset (sent via SMTP, or printed to console in dev mode)
- Passwords hashed securely with bcrypt (via passlib)

### 📊 Observability
- Every query is logged with full pipeline timing: retrieval time, re-rank time, LLM time, and total time
- Confidence scores calculated from cross-encoder results (sigmoid-normalized)
- Retrieved and re-ranked chunks are saved per query for easy debugging
- Dashboard shows live KPIs, a 14-day activity chart, and a breakdown by document type

### 🎨 Polished UI
- Custom deep-space dark theme — not a generic template
- Collapsible sidebar navigation
- Drag-and-drop file uploads with live status updates
- Streaming chat with expandable source panels and relevance score bars
...
In plain terms: the re-ranker gives each retrieved chunk a relevance score. We average the top 3 scores and convert that into a 0–100 confidence percentage using a sigmoid function.
...
| `POST` | `/auth/forgot-password` | Send a one-time password (OTP) to email |
| `POST` | `/auth/verify-otp` | Verify OTP and set a new password |
...
| `GET` | `/collections/{id}` | Get details for a specific collection |
| `PATCH` | `/collections/{id}` | Update a collection's name or description |
| `DELETE` | `/collections/{id}` | Delete a collection along with its documents and vectors |
...
| `POST` | `/documents/{id}/reindex` | Delete existing vectors and re-run ingestion |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `POST` | `/rag/ask` | Run the full pipeline and return a complete response with citations |
| `POST` | `/rag/ask/stream` | Streaming response (SSE) — tokens arrive in real time |
| `GET` | `/rag/logs` | View query history (filterable by collection) |
| `GET` | `/rag/logs/{id}` | View full log details, including chunks and context |
...
| `GET` | `/dashboard/activity?days=14` | Daily query counts for the activity chart |
...
|-------|------|-------------|
| `/login` | LoginPage | Email/password login with auto-redirect |
| `/register` | RegisterPage | Account creation with password strength indicators |
| `/forgot-password` | ForgotPasswordPage | Two-step OTP-based password reset |
| `/dashboard` | DashboardPage | KPI cards, activity chart, document-type breakdown, recent queries |
| `/collections` | CollectionsPage | Collection grid with create/delete options |
| `/collections/:id` | CollectionDetailPage | Collection stats, document list, and configuration panel |
| `/collections/:id/documents` | DocumentsPage | Drag-and-drop upload, status table, chunk viewer |
| `/ask` | AskPage | Streaming RAG chat with source panels and timing info |
| `/history` | HistoryPage | Query logs with expandable pipeline details |
| `/settings` | SettingsPage | Profile, pipeline defaults, and system information |

---

## Design System

RAG Forge uses a custom dark design system built on Tailwind CSS with semantic design tokens — meaning colors and styles are named by purpose, not just by appearance, making the UI consistent and easy to theme.
...
- **UI text**: [Inter](https://rsms.me/inter/) — weights 300, 400, 500, 600, 700, 800
- **Data/code**: [JetBrains Mono](https://www.jetbrains.com/lp/mono/) — weights 400, 500, 600

### Component Classes (global CSS)

| Class | Purpose |
|-------|---------|
| `.btn-primary` | Indigo glow button with hover lift effect |
| `.btn-secondary` | Bordered button that highlights indigo on hover |
| `.btn-ghost` | Transparent button, highlights on hover |
| `.btn-danger` | Red button for destructive actions |
| `.card` | Dark card with border and shadow |
| `.card-hover` | Clickable card with an indigo glow on hover |
| `.input` | Dark input field with an indigo focus ring |
...
- [ ] Configure `SMTP_*` variables so OTP emails actually get delivered
- [ ] Set `FRONTEND_URL` to your real domain in the backend `.env`
- [ ] Update the CORS `allow_origins` list in `main.py`
- [ ] Mount persistent volumes for `chroma_db/`, `uploads/`, and `ragforge.db`
- [ ] Set `MAX_FILE_SIZE_MB` based on your server's capacity
- [ ] Put a reverse proxy with SSL in front (Let's Encrypt or Caddy work well)
...
- The backend auto-reloads when you use the `--reload` flag
- Frontend hot-reloading works out of the box via Vite
- Use the `/docs` Swagger UI to test API endpoints directly in the browser
- In dev mode, OTP codes are printed to the backend console — no SMTP setup required
- ChromaDB and SQLite are file-based, so deleting `chroma_db/` and `ragforge.db` gives you a clean slate
